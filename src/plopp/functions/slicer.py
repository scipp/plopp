...
def slicer(
    ...
    **kwargs:
    ...
    from ..widgets import Box, dimslicer

    dimsl = dimslicer(ds, keep=keep)

    figure = figure1d if len(dimsl.keep) == 1 else figure2d

    # if len(keep) == 1:
    #     figure = figure1d(*self.slice_nodes, crop=crop, **kwargs)
    # elif len(keep) == 2:
    #     figure = figure2d(*self.slice_nodes, crop=crop, **kwargs)

    # from ..widgets import Box

    return Box([figure(*dimsl.output_nodes, crop=crop, **kwargs), dimsl])
