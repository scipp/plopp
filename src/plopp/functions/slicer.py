...
#             sizes={dim: size
#                    for dim, size in da.sizes.items() if dim not in keep},
#             coords=da.meta)
...
def slicer(obj: Union[VariableLike, ndarray],
    ...
    **kwargs:
    ...
    if len(keep) == 0:
        raise ValueError('Slicer plot: the list of dims to be kept cannot be empty.')

    from ..widgets import SliceWidget, slice_dims, Box

    data_nodes = [input_node(da) for da in ds.values()]

    slider = SliceWidget(
        sizes={dim: size
               for dim, size in ds.sizes.items() if dim not in keep},
        coords=ds.meta)
    slider_node = widget_node(slider)
    slice_nodes = [slice_dims(data_node, slider_node) for data_node in data_nodes]
    if len(keep) == 1:
        fig = figure1d(*slice_nodes, crop=crop, **kwargs)
    elif len(keep) == 2:
        fig = figure2d(*slice_nodes, crop=crop, **kwargs)

    return Box([fig, slider])
