...
class View2d(View):
    ...
    **kwargs:
    ...
    def update(self, new_values: sc.DataArray, key: str, xdim: str, ydim: str):
        """
        ...
        """
        # if new_values.ndim != self._allowed_dims:
        #     raise ValueError(
        #         f"View can only be used to plot {self._allowed_dims}-D data."
        #     )
        ...
        if key not in self.artists:
            artist = getattr(backends, self._illustrator)(
                canvas=self.canvas,
                data=new_values,
